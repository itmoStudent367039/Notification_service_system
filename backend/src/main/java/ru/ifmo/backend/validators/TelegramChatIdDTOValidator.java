...
public class TelegramChatIdDTOValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    ...
    if (tgId.isPresent()) {
      Optional<Person> personByTelegramChatId =
          peopleService.findByTelegramChatId(person.getTelegramChatId());

      if (personByTelegramChatId.isPresent()
          && (!Objects.equals(personByTelegramChatId.get().getId(), person.getId()))) {
